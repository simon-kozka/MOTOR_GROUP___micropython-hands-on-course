...
parser = argparse.ArgumentParser(description="IMU Data Augmentation Script")
parser.add_argument("--working_file", type=str, required=True, help="Path to working segment CSV")
parser.add_argument("--impact_folder", type=str, required=True, help="Path to impact CSV folder")
parser.add_argument("--output_data_file", type=str, default="IMU-Data.data", help="Output IMU data file")
parser.add_argument("--output_label_file", type=str, default="Live-Labeling.label", help="Output label file")

args = parser.parse_args()

working_file = args.working_file
impact_folder = args.impact_folder
...
# ===== Combine data =====
final_data = pd.concat(all_data, ignore_index=True).sort_values("time").reset_index(drop=True)

# ===== Resample to 400 Hz precisely =====
fs = 400.0  # Hz
dt = 1.0 / fs
t_start = final_data["time"].iloc[0]
t_end = final_data["time"].iloc[-1]

# New uniform time vector
t_uniform = np.arange(t_start, t_end, dt)

# Interpolate x, y, z to new timestamps
final_data_uniform = pd.DataFrame({
    "time": t_uniform,
    "x": np.interp(t_uniform, final_data["time"], final_data["x"]),
    "y": np.interp(t_uniform, final_data["time"], final_data["y"]),
    "z": np.interp(t_uniform, final_data["time"], final_data["z"])
})

# ===== Resample labels to match 400 Hz =====
resampled_labels = []
for lbl in all_labels:
    start, length, label_type, confidence, comment = lbl
    end = start + length

    # Find closest indices in t_uniform
    start_idx = np.searchsorted(t_uniform, start)
    end_idx = np.searchsorted(t_uniform, end)

    if start_idx >= len(t_uniform):
        continue  # skip if start beyond end
    if end_idx > len(t_uniform):
        end_idx = len(t_uniform)

    new_start = t_uniform[start_idx]
    new_length = t_uniform[end_idx-1] - new_start if end_idx > start_idx else dt

    resampled_labels.append([new_start, new_length, label_type, confidence, comment])

# ===== Save outputs =====
final_data_uniform.to_csv(output_data_file, index=False, float_format="%.8f")
print(f"Saved augmented IMU data at 400Hz -> {output_data_file}")
...
print(f"Saved label file aligned to 400Hz -> {output_label_file}")
